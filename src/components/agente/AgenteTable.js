...
function createDataAgente(usuario, folio) {
  return {
    id: usuario.id,
...
const EnhancedTableToolbar = (props) => {
  ...
  const { numSelected, setConfigCreateAgente, setOpenCreateModal } = props;

  const handleClickCreate = () => {
    setConfigCreateAgente({
      type: 'CREATE',
      item: null
    })
...
export default function AgenteTable({
  setOpenCreateModal,
  setOpenDelete,
  setDeleteAgente,
  setConfigCreateAgente,
  ...
  const getAgentes = async () => {
    ...
    const agentes = await as.getAgentes()
    setRows(agentes.map(a => createDataAgente(a.usuario, a.folio)));
    setLoading(false);
  }
